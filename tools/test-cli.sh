...
test ! -s cli-test-stderr
python3 -c 'import json,sys; v=json.load(sys.stdin); assert v["version"] == 2; assert v["transaction_backend"] == "native"; assert v["operations"] == ["resolve-lock", "create", "customize", "update", "recover"]; assert v["exact_lock_schema"].endswith("exact-closure-lock-v2"); assert v["provenance_schema"].endswith("native-transaction-provenance-v1"); assert v["recovery"] == "disposable_or_recoverable"; assert not v["invokes_apt"] and not v["invokes_dpkg"]' <cli-test-stdout
for arguments in \
    "--transaction-backend" \
