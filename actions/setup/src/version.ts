...
function parseVersion(value: string, label: string, requirePrefix: boolean): ResolvedVersion {
  ...
  if (!EXACT_SEMVER.test(version)) {
    throw new SetupError(
      `${label} must be an exact SemVer such as v0.3.0 or v0.3.0-rc.1; ranges and "latest" are not allowed`,
    );
  }
  try {
...
export function resolveVersion(explicitVersion: string, actionRef: string): ResolvedVersion {
  if (explicitVersion.length > 0) {
    return parseVersion(explicitVersion, 'debz-version', false);
  }
  if (actionRef.length === 0) {
    throw new SetupError(
      'debz-version is required when the action ref is unavailable; commit- and branch-pinned actions must set it explicitly',
    );
  }
