...
export const STATE_CACHE_PATH = 'debz-cache-path';
export const STATE_CACHE_KEY = 'debz-cache-key';
export const STATE_EXPECTED_ROOT = 'debz-cache-expected-root';
export const STATE_ASSET_SIZE = 'debz-cache-asset-size';
export const STATE_ARCHIVE_DIGEST = 'debz-cache-archive-digest';
export const STATE_VERSION = 'debz-cache-version';

const CACHE_VERSION = createHash('sha256')
  .update('debz-cli-raw-gzip-cache-v1')
  .digest('hex');
...
export class ActionsCacheV2 implements CacheAdapter {
    ...
    try {
      const download = await this.requester({
        ...
        headers: {
          ...
          'User-Agent': 'cataggar-debz-setup-action',
        },
      ...
      if (download.statusCode !== 200) {
        throw new Error(
          `exact cache download failed with HTTP ${download.statusCode}`,
        );
      }
      ...
      throw new Error('exact cache hit could not be downloaded', {
        cause: error,
      });
    }
  }
  ...
  async saveCache(key: string, archive: Buffer): Promise<number> {
    ...
    const uploadURL = validateBlobURL(signedURL);
...
export function cacheKey(target: Target, tag: string, digest: string): string {
  return `debz-cli-${CACHE_SCHEMA}-${target}-musl-static-${tag}-${digest}`;
}
