...
4. Run the separate `CI` workflow manually with the pinned immutable Ubuntu snapshot URI and suite selected for the release. Require both native real-snapshot acceptance jobs to pass and retain their architecture-tagged `vendor-state-inventory-v1.json` review artifacts; these captures are evidence for future pinning, not a claim that repository-pinned vendor manifests already exist. Normal pull-request CI intentionally does not download real Ubuntu archives.
