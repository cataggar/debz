...
4. Run the separate `CI` workflow manually with the pinned immutable Ubuntu snapshot URI and suite selected for the release. Require both native real-snapshot acceptance jobs to pass, retain their architecture-tagged `vendor-state-inventory-v1.json` review artifacts, and compare them with the indexed references under `tools/fixtures/vendor-state/`. Refresh those references only in a separately reviewed change that records the exact workflow run, source commit, artifact and manifest digests, capture schema version, and inventory differences. Normal pull-request CI intentionally does not download real Ubuntu archives.
