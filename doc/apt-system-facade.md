...
path digest, profile and reference, semantic request, exact lock, reviewed
mutation status, nonce,
...
After claim validation, every non-fatal exit before atomic lower-protocol
handoff exact-releases the claim. Profile disappearance or replacement and
outer-state/evidence drift cancel the original profile-path-bound claim before
returning a fail-closed diagnostic; a foreign profile path cannot clear it.
Fatal allocation, contract, and invariant failures propagate without
reconciliation or claim removal. JSON cancellation is generated by the same
