...
sorts normalized repositories, rejects duplicates, and returns:
...
are ordered by package, architecture, Debian version, effective priority,
repository ID, and record index. `importInto` imports the same deterministic
repository sequence into `SolverContext`. Candidate precedence is package,
architecture, effective priority, Debian version, repository ID, then record
index.
