...
- [Crash-safe root mutation layer](root-mutation.md) documents the versioned mutation journal, the chained write-ahead progress log, preflight preconditions, the durability boundaries, recovery guarantees, root-local staging and backups, and the package-database and archive adapters.
- [Native unpack and file ownership](native-unpack.md) documents the planning-only unpack slice: authenticated archive/database revalidation, canonical ownership, `Replaces`, `Multi-Arch`, merged-`/usr`, immutable filesystem/database descriptions, and typed lifecycle handoffs.
...
- [Pinned dpkg/update-alternatives reference](dpkg-alternatives-reference.md) documents the bounded amd64 record, link, package lifecycle, failure, attack, atomicity, and recovery oracle for the 14 reviewed vendor groups without admitting native support.
