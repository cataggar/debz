...
| `var/lib/dpkg/diversions` | Parse and honor complete three-line records with exact logical/physical routing and literal package exemptions. Atomic replacements reload the invocation cache; valid in-place edits preserve its loaded records. Malformed/conflicting or reserved-path records remain refused. Mid-unpack old-postrm updates still require recovery (follow-up [#192](https://github.com/cataggar/debz/issues/192)). |
