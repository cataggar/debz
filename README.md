...
`debz` is an in-development, embeddable Debian-family package manager and CLI written in Zig. It is designed around explicit inputs, bounded parsing, authenticated repository metadata, deterministic dependency solving, and transaction planning while using `dpkg` as the eventual transaction backend. It does not yet modify packages or repositories.
...
See the [documentation index](doc/README.md) for current capabilities, security boundaries, authenticated refresh usage, the supported OpenPGP profile, and API/schema references.
